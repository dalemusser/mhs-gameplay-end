...
// then walks back. Entrances and exits run CONCURRENTLY on separate lanes
// (LANE_OUT_Z / LANE_BACK_Z) so speaker hand-offs overlap instead of serializing.
const STAGE = {
...
// Two-lane crossing traffic: the OUTGOING speaker crosses on a lane nearer the
// audience while the RETURNING one crosses nearer the homes — so both can walk at
// the same time (overlapped transitions) and pass each other without colliding.
const LANE_OUT_Z = 2.6;                                // walk-out lane (downstage)
const LANE_BACK_Z = 1.9;                               // walk-back lane (upstage)
...
// back-lane for returns, out-lane for entrances (see LANE_*_Z).
const sendBack = c => { if (c.state === 'out' || c.state === 'speak') { c.state = 'back'; c.curve = { p0: c.holder.position.clone(), p1: new V3(c.home.x, 0, LANE_BACK_Z), p2: c.home.clone() }; c.t = 0; } };
const startOut = c => { c.state = 'out'; c.curve = { p0: c.holder.position.clone(), p1: new V3(c.home.x, 0, LANE_OUT_Z), p2: SPEAK_MARK.clone() }; c.t = 0; };
...
let finished = false;
function finish() { finished = true; bubble.classList.remove('show'); setShot(SHOTS.wide); music.bed(false); stopLineAudio(); confetti.stop(); applause(false, 0.8); updateNextLabel(); }
...
scene.onBeforeRenderObservable.add(() => {
  ...
  if (celebWaiting) {
    ...
    if (anyWalking) celebReadyAt = now + celebDur;             // keep pushing while someone's still returning
      ...
      celebWaiting = false;
      // an `ending` celebration fades to black and finishes instead of advancing;
      // the applause fades out in step with the visual fade
      if (beats[idx] && beats[idx].ending) { fadeEl.style.opacity = '1'; applause(false, 2.4); endTimer = setTimeout(finish, 2600); }
      else advance();
    }
  }
