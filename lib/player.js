 ...
 *
 * v8 (v0.1.9): holo images load with crossOrigin = 'anonymous' (+ a tint
 * readback that cannot throw) — see holoImage(). Under the stratahub host
 * page the bundle's same-origin content path redirects to the CDN, which
 * tainted the wall canvas and froze the show at Toppo's first line.
 *
 * v9 (v0.2.0): the optimized asset set (tools/optimize.sh — 15 clips per rig,
 * resampled, WebP textures ≤ 1024 px; 64 kbps voice, 128 kbps music, holo
 * JPEGs ≤ 1280 px, WebP logo). GLB_V 17, HOLO_V 3, AUDIO_V 2, logo → .webp.
 *
 * Lineage: v9 SHIPPED in dist/v0.2.0 (live 2026-09-22) and archived as
 * lib/player_v9.js + ceremony_v9.html; this plain file is the v10 working copy.
 * v8 shipped in dist/v0.1.9 (archived as _v8), v7 in dist/v0.1.8 (_v7).
 * v7 shipped in dist/v0.1.8 (live 2026-09-22, archived as _v7).
 * v7 was cloned from v6 (?v=3, shipped in dist/v0.1.7 = live 2026-09-10). Content is shared with v6 for now (ceremony-definition_v6.js +
 ...
 *
 * v10 (v0.2.1) = v9 + AUTOSTART: opts.autostart plays the show as soon as the
 * bundle is ready, with no Begin click, when the browser lets the page play
 * sound (a silent probe decides; see autoBegin). When it refuses — no user
 * activation carried into this page — the Begin button stays as the gesture,
 * with a one-line note. Every arrival from StrataHub autostarts (the student
 * just pressed Continue in the game; a replay just tapped Watch). Dale, 2026-09-24.
 *
...
function holoImage(rel) {
  ...
  if (!e) {
    const img = new Image();
    // CORS-enabled load (v8): the host page serves the bundle through a
    // same-origin path that REDIRECTS to the CDN, and an image that crosses an
    // origin during a redirect is tainted for canvas use unless it was a CORS
    // request — drawing it on the wall canvas then made Babylon's texture
    // upload throw a SecurityError inside the first beat with holo images
    // (Toppo's opening line), freezing the show right after the cold open.
    // The CDN answers CORS for the host origins; standalone (same origin) this
    // is a no-op.
...
function extractTint(img) {
  try { return extractTintUnsafe(img); } catch (e) { return null; }   // a tainted image must never throw here
}
function extractTintUnsafe(img) {
  try {
...
const EMBLEM_LOGO = { src: BASE + 'assets/logo/mhs-logo.webp?v=2', aspect: 1378 / 1141 };
...
const GLB_V = 17;       // v0.2.0 optimized rigs (tools/optimize.sh); 16 = restored corrected Mocap Walk cycle
...
const LINE_AUDIO = {}, AUDIO_V = 2, AUDIO_DIR = BASE + 'assets/audio_v6/';
...
// start gate: nothing makes sound until the show begins — on a Begin click
// (the gesture that unlocks audio), or straight away under opts.autostart when
// the browser already lets the page play sound.
function beginShow() {
  if (!window.__ready || showBegun) return;
  showBegun = true;
...
let showBegun = false;
document.getElementById('startBtn').addEventListener('click', beginShow);

// AUTOSTART (opts.autostart): can this page play sound without a click? The
// browsers decide per page — Chrome lets a page reached by a user's click
// (the game's Continue, the launcher's Watch) play, Safari/iPad usually do
// not — so ask: play a 100 ms silent WAV. Allowed → begin now; refused
// (NotAllowedError) → the Begin button stays as the gesture, with a note.
// Any other failure (an odd browser that cannot play the probe) also falls
// back to Begin: never a stuck black page.
const SILENT_WAV = 'data:audio/wav;base64,' + 'UklGRkMDAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YR8DAACA' + 'gICA'.repeat(266);
function probeAudio() {
  return new Promise((resolve, reject) => {
    let a;
    try { a = new Audio(SILENT_WAV); } catch (e) { reject(e); return; }
    a.volume = 0.01;                          // audible in policy terms (muted playback is always allowed and would prove nothing)
    let p;
    try { p = a.play(); } catch (e) { reject(e); return; }
    if (!p || !p.then) { resolve(); return; } // ancient API: no promise, no policy
    p.then(() => { try { a.pause(); } catch (e) {} resolve(); }, reject);
  });
}
function autoBegin() {
  const sb = document.getElementById('startBtn');
  sb.disabled = true; sb.textContent = 'Starting…';
  probeAudio().then(beginShow, err => {
    sb.disabled = false; sb.textContent = '▶ Begin Ceremony';
    const ov = document.getElementById('startOverlay');
    if (ov && !document.getElementById('startNote')) {
      const note = document.createElement('div');
      note.id = 'startNote';
      note.textContent = 'Tap Begin to start.';
      note.style.cssText = 'margin-top:14px;color:#cfe0ff;font:15px/1.4 system-ui,sans-serif;text-align:center;';
      ov.appendChild(note);
    }
    console.info('ceremony: autostart deferred to Begin (' + ((err && err.name) || err) + ')');
  });
}
...
Promise.all(order.map(loadChar)).then(() => {
  ...
  // the show starts when Begin is clicked (the gesture unlocks audio) — or
  // right now under opts.autostart when the browser allows it (autoBegin)
  if (opts.autostart) autoBegin();
