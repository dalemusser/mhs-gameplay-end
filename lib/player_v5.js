/*
 * MHSCeremonyPlayer v5 — the new WORKING version, cloned from v4 (shipped in
 * dist/v0.1.6, frozen as rollback). All new changes land HERE; bump the
 * player_v5.js?v=N buster in ceremony_v5.html on EVERY edit.
 *
...
function walkStatus() {                       // the lower-third never sits empty during a hand-off
  bubbleTextEl.innerHTML = '';
  const s = document.createElement('span');
  s.className = 'status'; s.textContent = 'Walking to the stage.';
...
function restartShow() {
  ...
  bubble.classList.remove('show'); bubbleTextEl.innerHTML = ''; speakerNameEl.textContent = '';
  setControls(false);
  updateNextLabel();
